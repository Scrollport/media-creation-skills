...
repository is the only editable source for the Media creation router, function
Skills, validation, evidence and releases. Function Skills remain independently
addressable inside the category repository; do not create one repository per
workflow.

The cross-category directory and private product monorepo consume generated,
commit-pinned exports from category repositories. They are compatibility and
distribution surfaces, not parallel editable sources; never edit a generated
export as if it were canonical.
